...
def get_probs_of_seq(seq):
  ...
  with torch.no_grad():
    for (sub_seq, next_word) in get_all_sub_strs(seq):
      ...
      word = tokenizer(next_word, return_tensors="pt").to(device)

      logits = model(**inputs).logits[:, -1, :]
      probs = torch.softmax(logits, dim=-1)
      position_in_probs_of_next_word = tokenizer(next_word, return_tensors="pt").to(device)['input_ids'][0][0]
      prob_of_next_word = probs[0][position_in_probs_of_next_word]
      prob_results.append((prob_of_next_word, next_word))
  return prob_results
